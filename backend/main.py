...
def login(data: schemas.UserBase, db: Session = Depends(get_db)):
    ...
    מחפש את הטלפון בעמודות phone ו-Phone2, תוך התעלמות ממקפים. אם לא נמצא, נוצר משתמש חדש.
    """
	# הטלפון מהלקוח (RSVPScreen) כבר מגיע נקי ממקפים, לכן data.phone נקי.
	# נבצע חיפוש שמתעלם ממקפים במסדי הנתונים.
	user = db.query(User).filter(
		sa.or_(
			sa.func.replace(User.phone, '-', '') == data.phone,
			sa.func.replace(User.Phone2, '-', '') == data.phone
		)
	).first()

	if not user:
		# אם המשתמש לא נמצא, יוצרים אחד חדש עם הטלפון הנקי שהתקבל.
...
def list_users(
	...
	"""
    החזר את כל המשתמשים, או – אם קיים פרמטר q – בצע חיפוש על שם, טלפון או טלפון 2, תוך התעלמות ממקפים בטלפונים.
    """
	qry = db.query(User)
	if q:
		# מנקה מקפים ממחרוזת החיפוש למקרה שמדובר בטלפון
		cleaned_q_for_phone = q.replace("-", "")
		phone_like_pattern = f"%{cleaned_q_for_phone}%"
		name_like_pattern = f"%{q}%"

		# חיפוש לפי שם (עם המחרוזת המקורית), או לפי טלפון/טלפון2 (עם מחרוזת נקייה)
		qry = qry.filter(
			sa.or_(
				User.name.ilike(name_like_pattern),
				sa.func.replace(User.phone, '-', '').ilike(phone_like_pattern),
				sa.func.replace(User.Phone2, '-', '').ilike(phone_like_pattern)
			)
		)
	return qry.all()
...
def create_user_endpoint(data: schemas.UserCreate, db: Session = Depends(get_db)):
	"""
    צור משתמש חדש (אם הטלפון לא קיים כבר, תוך התעלמות ממקפים).
    """
	# הטלפון מהלקוח (AdminScreen) כבר מגיע נקי ממקפים.
	# נבצע בדיקה למניעת כפילויות שמתעלמת ממקפים במסד הנתונים.
