...
const NavBar: React.FC<NavBarProps> = ({ setIsAdminLoggedIn }) => {
    ...
    return (
      <Box as="header" bg={bg} position="sticky" top="0" zIndex="1000" boxShadow="sm" h={NAV_HEIGHT} dir="rtl">
        <Container maxW="7xl" h="full">
          <Flex as="nav" h="full" align="center" justify="center">
            <Button colorScheme="red" variant="ghost" onClick={handleLogout}>התנתק</Button>
          </Flex>
        </Container>
      </Box>
    );
  ...
  const phoneInputField = (
    <VStack w="full" spacing={3}>
      ...
      />
      <HStack w="full">
        <Button w="50%" sx={glassmorphismStyle} isLoading={isLoading} onClick={handleAdminLogin}>כניסה</Button>
        <Button w="50%" variant="ghost" onClick={() => { setPhoneInput(""); adminModal.onClose(); }}>ביטול</Button>
      </HStack>
    </VStack>
  );
  ...
  return (
    <>
      {/* ── Desktop ── */}
      <Box display={{ base: "none", md: "block" }} as="header" bg={bg}
        position="sticky" top="0" zIndex="1000" boxShadow="sm" h={NAV_HEIGHT} dir="rtl">
        <Container maxW="7xl" h="full">
      ...
      {/* ── Mobile Floating ── */}
      <VStack spacing={3} position="fixed" bottom="24px" right="24px"
        zIndex="1050" display={{ base: "flex", md: "none" }} alignItems="flex-start">
        <IconButton aria-label="פתיחת תפריט"
        ...
        <MotionButton as={ChakraLink} href="#rsvp"
          _hover={{ textDecoration: "none", transform: "scale(1.05)" }}
          h="56px" borderRadius="full" px={6} color={primaryTextColor} shadow="lg" sx={glassmorphismStyle}
      ...
      <Drawer isOpen={drawer.isOpen} placement="right" onClose={drawer.onClose} size="xs">
        ...
        <DrawerContent dir="rtl" sx={glassmorphismStyle} color={primaryTextColor}>
          <DrawerHeader borderBottomWidth="1px" borderColor="rgba(255,255,255,0.2)">
            <Button variant="ghost" leftIcon={<CloseIcon />} color="currentcolor"
              _hover={{ bg: "rgba(255,255,255,0.1)" }} onClick={drawer.onClose}>סגור</Button>
          </DrawerHeader>
          <Box as="nav" pt={6} px={4}>
            <VStack spacing={4}>
              {navLinks.map((l) => (
                <ChakraLink key={l.href} href={l.href} w="full" textAlign="center"
                  py={3} rounded="md" fontWeight="semibold" color={primaryTextColor}
                  _hover={{ bg: "rgba(255,255,255,0.1)" }} onClick={drawer.onClose}>
                  {l.label}
                </ChakraLink>
              ))}
              {adminModal.isOpen ? phoneInputField : (
                <Button variant="outline" borderColor={primaryTextColor} w="full"
                  color="currentcolor" _hover={{ bg: "rgba(255,255,255,0.1)" }}
                  onClick={adminModal.onOpen}>אדמין</Button>
              )}
            </VStack>
          </Box>
        </DrawerContent>
      ...
      <Modal isOpen={adminModal.isOpen} onClose={adminModal.onClose} isCentered>
        ...
        <ModalContent dir="rtl">
          ...
          <ModalFooter gap={3}>
            <Button colorScheme="brand" isLoading={isLoading} onClick={handleAdminLogin}>כניסה</Button>
...
const HandDrawnArrow = (props: any) => (
  <Icon viewBox="0 0 24 24" fill="none" stroke="currentColor"
    ...
    <path d="M12 3v18" /><path d="M19 14l-7 7-7-7" />
  </Icon>
);
...
const ScrollDownIndicator = () => {
  ...
  return (
    <>
      <MotionDiv style={{ opacity: scrollOpacity }} position="fixed" bottom="20px" left="15%"
        transform="translateX(-50%)" zIndex={900} color={color} pointerEvents="none">
      ...
      <MotionDiv as="a" href="#singles" style={{ opacity: singlesOpacity }}
        position="fixed" bottom="20px" left="15%" transform="translateX(-50%)"
        zIndex={900} color={color} cursor="pointer"
        _hover={{ transform: "translateX(-50%) scale(1.1)" }} transition="transform 0.2s">
        <MotionDiv animate={{ y: [0, 12, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}>
          <VStack spacing={0}>
            <Text fontSize="s" fontWeight="bold" mb={1} fontFamily="heading" textAlign="center">רווקים/ות?</Text>
            <HandDrawnArrow w={8} h={10} />
          </VStack>
        </MotionDiv>
      </MotionDiv>
...
const Home: React.FC = () => {
  ...
  return (
    <Container maxW="container.xl" py={{ base: 6, md: 10 }} position="relative">
      <ScrollDownIndicator />
      <Text color="brand.900" fontSize="xs" position="absolute" top={2} right={12}>בסייעתא דשמיא!</Text>
 ...
 * ------------------------------------------------------------------ */
const App: React.FC = () => {
  const [isAdminLoggedIn, setIsAdminLoggedIn] = useState<boolean>(isTokenLikelyValid);
  ...
  return (
    <Router>
      <Box position="fixed" top={0} left={0} w="100vw" h="100vh"
        bgGradient={gradient} zIndex={-1} pointerEvents="none" />
      <Flex direction="column" minH="100vh" color={textClr} dir="rtl">
        ...
        <Box as="footer" id="end" bg="bg.canvas" py={4} textAlign="center">
          <Text fontSize="sm" color="text.secondary">© 2025 טובת &nbsp;&amp;&nbsp; ירדן</Text>
        </Box>
      </Flex>
    </Router>
  );
